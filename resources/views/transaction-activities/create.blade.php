...
@section('title', 'Add Transaction Activity')
@section('page-title', 'Add Transaction Activity')

@section('content')
<div class="card shadow-sm rounded-4">
    <div class="card-body">
        <form action="{{ route('admin.transaction-activities.store') }}" method="POST">
            @csrf
                ...
                <div class="col-md-6">
                    ...
                    <select name="ebook_id" id="ebook_id" class="form-select @error('ebook_id') is-invalid @enderror" required>
                        ...
                        @foreach($ebooks as $ebook)
                            <option value="{{ $ebook->id }}" {{ old('ebook_id') == $ebook->id ? 'selected' : '' }}>{{ $ebook->title }}</option>
                        @endforeach
                    </select>
                    @error('ebook_id')
                        <div class="invalid-feedback">{{ $message }}</div>
            ...
            <div class="mb-3">
                ...
                <input type="text" name="activity_type" id="activity_type" value="{{ old('activity_type') }}" class="form-control @error('activity_type') is-invalid @enderror" required>
                @error('activity_type')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
